...
namespace Janphe.Fantasy.Map
{
    ...
    internal partial class HeightmapGenerator
    {
        ...
        public void generate()
        {
            p = grid.points;
            cells = grid.cells;
            cells.r_height = new byte[p.Count];
        ...
        private void addHill(string count_s, string height, string rangeX, string rangeY)
        {
            ...
            void addOneHill()
            {
                var change = new byte[cells.r_height.Length];
                int limit = 0, start;
                ...
                } while (cells.r_height[start] + h > 90 && limit < 50);
                ...
                while (queue.Count > 0)
                {
                    ...
                    foreach (var c in cells.r_neighbor_r[q])
                    {
                ...
                cells.r_height = cells.r_height.Select((o, i) => lim(o + change[i])).ToArray();
            }
        }
        ...
        private void addPit(string count_s, string height, string rangeX, string rangeY)
        {
            ...
            void addOnePit()
            {
                var used = new byte[cells.r_height.Length];
                int limit = 0, start;
                double h = lim(getNumberInRange(height));
                ...
                do
                {
                    ...
                    start = grid.findGridCell(x, y);
                    limit++;
                } while (cells.r_height[start] < 20 && limit < 50);
            ...
            void addOneRange()
            {
                ...
                int[] getRange(int cur, int end)
                {
                    ...
                    while (cur != end)
                    {
                        ...
                        foreach (var e in cells.r_neighbor_r[cur])
                        {
                ...
                while (queue.Count > 0)
                {
                    ...
                    {
                        foreach (var i in cells.r_neighbor_r[f])
                        {
                ...
                for (int d = 0; d < range.Length; ++d)
                {
                    ...
                    {
                        var idx = D3.scan(cells.r_neighbor_r[cur], (a, b) => old[a] - old[b]);
                        var min = cells.r_neighbor_r[cur][idx];// downhill cell
        ...
        private void addStrait(string width_s, string direction = "vertical")
        {
            ...
            while (width > 0)
            {
                ...
                {
                    foreach (var e in cells.r_neighbor_r[r])
                    {
                        if (used[e] == 1) continue;
                        used[e] = 1;
                        query.Add(e);
        ...
        private void modify(string range, double add, double mult, double power = 0)
        {
            ...
            grid.cells.r_height = grid.cells.r_height.Select(h => h >= min && h <= max ? mod(h) : h).ToArray();
