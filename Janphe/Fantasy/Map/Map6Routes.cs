...
namespace Janphe.Fantasy.Map
{
    internal class Map6Routes
    {
        ...
        {
            var pack = map.pack; var cells = pack.cells;
            ...
            foreach (var i in cells.i) cells.s[i] += (short)(cells.road[i] / 2); // add roads to suitability score
            return paths;
        }
            ...
            foreach (var f in features)
            {
                ...
                for (var i = 0; i < isle.Length; ++i)
                {
                    ...
                    else
                    {
                        ...
                        if (cells.road[b.cell] > 0) continue;
                    ...
                    if (path != null) paths.AddRange(path);
                }
            }
            return paths;
        }
            ...
            foreach (var f in bodies)
            {
                ...
                if (pack.features[f].type != "lake")
                {
                    ...
                    if (portsOnIsland.Length > 3)
                    {
                        ...
                        var ret = findOceanPath(opposite, first); var from = ret.Item1; var exit = ret.Item2;
                ...
                {
                    var ret = findOceanPath(farthest, first); var from = ret.Item1; var exit = ret.Item2;
                ...
                foreach (var p in ports)
                {
                    ...
                    var ret = findOceanPath(p.cell, first, true); var from = ret.Item1; var exit = ret.Item2;
        ...
        private Tuple<Dictionary<int, int>, int> findLandPath(int start, int exit = -1, bool toRoad = false)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                if (toRoad && cells.road[n] > 0) { return Tuple.Create(from, n); }

                foreach (var c in cells.r_neighbor_r[n])
                {
                    if (cells.r_height[c] < 20) continue;// ignore water cells
                    ...
                    var totalCost = Utils.rn(tmp, 6);

                    if ((from.ContainsKey(c) && 0 != from[c]) || (cost.ContainsKey(c) && totalCost >= cost[c])) continue;
                    from[c] = n;
                    if (c == exit) { return Tuple.Create(from, exit); }
        ...
        private Tuple<Dictionary<int, int>, int> findOceanPath(int start, int exit = -1, bool toRoute = false)
        {
            ...
            {
                var next = queue.pop(); var n = next.e; var p = next.p;
                if (toRoute && n != start && cells.road[n] > 0) { return Tuple.Create(from, n); }

                foreach (var c in cells.r_neighbor_r[n])
                {
                    if (cells.r_height[c] >= 20) continue; // ignore land cells
        ...
        private List<int[]> restorePath(int start, int end, string type, Dictionary<int, int> from)
        {
            ...
            if (type == "ocean" || 0 == cells.road[prev]) segment.Add(end);
            if (0 == cells.road[prev]) cells.road[prev] = (ushort)score;

            for (int i = 0, limit = 1000; i < limit; i++)
            {
                if (0 == from[current]) break;
            ...
            if (segment.Count > 1) path.Add(segment.ToArray());
            return path;
        }
