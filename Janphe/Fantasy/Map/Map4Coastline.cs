...
namespace Janphe.Fantasy.Map
{
    internal class Map4Coastline
    {
        ...
        {
            if (t == -1 && cells.r_near_border[i])
                return Array.Find(cells.v[i], v => Array.Exists(vertices.c[v], c => c >= n)); // map border cell
            var filtered = cells.r_neighbor_r[i].Where(c => cells.t[c] == t).ToArray();
            var min = filtered.Length > 0 ? filtered.Min() : -1;
            var index = Array.FindIndex(cells.r_neighbor_r[i], c => c == min);
