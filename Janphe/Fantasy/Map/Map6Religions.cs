...
namespace Janphe.Fantasy.Map
{
    ...
    internal partial class Map6Religions
    {
        ...
        {
            var cells = pack.cells; var states = pack.states; var cultures = pack.cultures;
            ...
            cultures.forEach(c =>
            {
                if (0 == c.i) { religions.push(new Religion { i = 0, name = "No religion" }); return; }
                if (c.removed) { religions.push(new Religion { i = c.i, name = "Extinct religion for " + c.name, color = Utils.getMixedColor(c.color, .1, 0), removed = true }); return; }
            ...
            var cultsCount = Math.Floor(Utils.rand(10, 40) / 100 * religionsInput);
            ...
            for (var i = 0; religions.Count < count && i < 1000; i++)
            {
                ...
                var ret = getReligionName(form, deity, center); var name = ret[0]; var expansion = ret[1];
                if (expansion == "state" && 0 == state) expansion = "global";
                if (expansion == "culture" && 0 == culture) expansion = "global";

                if (expansion == "state" && Random.NextDouble() > .5) center = states[state].center;
                if (expansion == "culture" && Random.NextDouble() > .5) center = cultures[culture].center;

                if (0 == cells.burg[center] && cells.c[center].some(c => cells.burg[c] != 0)) center = cells.c[center].find(c => cells.burg[c] != 0);
                var x = cells.p[center][0]; var y = cells.p[center][1];

                var s = spacing * Utils.gauss(1, .3, .2, 2, 2); // randomize to make the placement not uniform
                if (religionsTree.find(x, y, s) != null) continue; // to close to existing religion

                // add "Old" to name of the folk religion on this culture
                var folk = religions.find(r => r.culture == culture && r.type == "Folk");
                if (folk != null && expansion == "culture" && folk.name.slice(0, 3) != "Old") folk.name = "Old " + folk.name;
            ...
            for (var i = 0; religions.Count < count + cultsCount && i < 1000; i++)
            {
                ...
                if (0 == cells.burg[center] && cells.c[center].some(c => 0 != cells.burg[c])) center = cells.c[center].find(c => 0 != cells.burg[c]);
                var x = cells.p[center][0]; var y = cells.p[center][1];
            ...
            religions.filter(r => r.type == "Organized").ToList().forEach(r =>
            {
                ...
                for (var i = 0; i < IMAX; i++)
                {
                    ...
                    if (0 == center) continue;
                    if (0 == cells.burg[center] && cells.c[center].some(c => 0 != cells.burg[c])) center = cells.c[center].find(c => 0 != cells.burg[c]);
                    double x = cells.p[center][0], y = cells.p[center][1];
                    if (religionsTree.find(x, y, spacing / 10) != null) continue; // to close to other
        ...
        {
            var cells = pack.cells; var religions = pack.religions;
        ...
        private void expandReligions(IList<Religion> religions)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                cells.c[n].forEach(e =>
                {
                    ...
                    {
                        if (cells.h[e] >= 20 && 0 != cells.culture[e]) cells.religion[e] = (ushort)r; // assign religion to cell
                        cost[e] = totalCost;
        ...
        private void expandHeresies(IList<Religion> religions)
        {
            ...
            while (queue.Count > 0)
            {
                var next = queue.pop(); int n = next.e, r = next.r, b = next.b; var p = next.p;
        ...
        private void checkCenters(IList<Religion> religions)
        {
            ...
            religions.filter(r => 0 != r.i).forEach(r =>
            {
                ...
                if (0 == religCells.Count()) return; // extinct religion
        ...
        private string generateMeaning()
        {
            ...
            if (a == "Number") return Utils.ra(@base["number"]);
            if (a == "Being") return Utils.ra(@base["being"]);
            if (a == "Adjective") return Utils.ra(@base["adjective"]);
            if (a == "Color + Animal") return Utils.ra(@base["color"]) + " " + Utils.ra(@base["animal"]);
            if (a == "Adjective + Animal") return Utils.ra(@base["adjective"]) + " " + Utils.ra(@base["animal"]);
            if (a == "Adjective + Being") return Utils.ra(@base["adjective"]) + " " + Utils.ra(@base["being"]);
            if (a == "Adjective + Genitive") return Utils.ra(@base["adjective"]) + " " + Utils.ra(@base["genitive"]);
            if (a == "Color + Being") return Utils.ra(@base["color"]) + " " + Utils.ra(@base["being"]);
            if (a == "Color + Genitive") return Utils.ra(@base["color"]) + " " + Utils.ra(@base["genitive"]);
            if (a == "Being + of + Genitive") return Utils.ra(@base["being"]) + " of " + Utils.ra(@base["genitive"]);
            if (a == "Being + of the + Genitive") return Utils.ra(@base["being"]) + " of the " + Utils.ra(@base["theGenitive"]);
            if (a == "Animal + of + Genitive") return Utils.ra(@base["animal"]) + " of " + Utils.ra(@base["genitive"]);
            if (a == "Adjective + Being + of + Genitive") return Utils.ra(@base["adjective"]) + " " + Utils.ra(@base["being"]) + " of " + Utils.ra(@base["genitive"]);
            if (a == "Adjective + Animal + of + Genitive") return Utils.ra(@base["adjective"]) + " " + Utils.ra(@base["animal"]) + " of " + Utils.ra(@base["genitive"]);
            return string.Empty;
        }
        ...
        {
            var cells = pack.cells;
            string random() { return Names.getCulture(cells.culture[center], 0, 0, "", 0); }
            string type() { return Utils.rw(types[form]); }
            string supreme() { return deity.split(" ,")[0]; }//(级别或地位) 最高的，至高无上的; (程度) 很大的，最大的;
            ...
            string culture() { return pack.cultures[cells.culture[center]].name; }

            var m = Utils.rw(methods);
            if (m == "Random + type") return new string[] { random() + " " + type(), "global" };
            if (m == "Random + ism") return new string[] { Utils.trimVowels(random()) + "ism", "global" };
            if (m == "Supreme + ism" && !string.IsNullOrEmpty(deity)) return new string[] { Utils.trimVowels(supreme()) + "ism", "global" };
            if (m == "Faith of + Supreme" && !string.IsNullOrEmpty(deity)) return new string[] { Utils.ra(new string[] { "Faith", "Way", "Path", "Word", "Witnesses" }) + " of " + supreme(), "global" };
            if (m == "Place + ism") return new string[] { place() + "ism", "state" };
            if (m == "Culture + ism") return new string[] { Utils.trimVowels(culture()) + "ism", "culture" };
            if (m == "Place + ian + type") return new string[] { place("adj") + " " + type(), "state" };
            if (m == "Culture + type") return new string[] { culture() + " " + type(), "culture" };
        ...
        private string getCultName(string form, int center)
        {
            ...
            string burg() { return Utils.trimVowels(pack.burgs[cells.burg[center]].name.split(" ,")[0]); }
            if (0 != cells.burg[center]) return burg() + "ian " + type();
            if (Random.NextDouble() > .5) return random() + "ian " + type();
