    ...
    internal class Map2Precipitation
    {
        ...
        public void generatePrecipitation()
        {
            ...
            if (northerly > 0)
            {
                ...
                var latModN = mapCoordinates.latT > 60 ? D3.mean(lalitudeModifier) : lalitudeModifier[bandN];
            ...
            if (southerly > 0)
            {
                ...
                var latModS = mapCoordinates.latT > 60 ? D3.mean(lalitudeModifier) : lalitudeModifier[bandS];
        ...
        private void passNext(int first, double maxPrec, int next, int steps)
        {
            ...
            for (int s = 0, current = first; s < steps; s++, current += next)
            {
                ...
                {
                    var outOfRange = (current + next) < 0 || (current + next) >= cells.r_height.Length;
                    if (!outOfRange && (cells.r_height[current + next] >= 20))
                    {
